...
class Paydatamodel extends CI_Model
{
	...
	function getPayDataByDay($fromTime,$toTime,$channel,$server,$version)
	{
		...
		for ($i=0;$i<$query->num_rows();$i++) {
			$fRow = array();
			$fRow["date"] = substr($PayDataRow->datevalue, 0, 10);
	...
	function getPayData($fromTime,$toTime,$channel,$server,$version)
	{
		...
		$sql = "SELECT
				...
				FROM
					".$dwdb->dbprefix('sum_basic_paydata')." s
				INNER JOIN ".$dwdb->dbprefix('dim_product')." p ON 
				p.product_id = $productId
				AND s.channel_name in('" . $channel_list . "')
				AND p.version_name in('" . $version_list . "')
				AND s.server_name in('" . $server_list . "')
     ...
     * @return Int pay data
     */
    function getTotalPayamount($appid, $channelid = 'all', $serverid = 'all', $versionname = 'all',$startdate,$enddate) {
        if ($channelid == 'all' && $serverid <> 'all' && $versionname <> 'all') {
            $sql = "SELECT
                    ...
                    FROM
                            razor_pay p
                    WHERE
                            p.pay_date >= '$startdate' AND p.pay_date <= '$enddate'
                    AND p.appId = '$appid'
                    #AND p.chId = '$channelid'
                    AND p.srvId = '$serverid'
                    AND p.version = '$versionname';";
