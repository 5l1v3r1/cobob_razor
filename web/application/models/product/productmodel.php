...
class ProductModel extends CI_Model
{
	 ...
	 * @return Int id
	 */
	function getProductIdByKey($productkey)
	{
		$this -> db -> select('product_id');
		$this -> db -> where('productkey', $productkey);
		$query = $this -> db -> get('channel_product');
		$row = $query -> first_row();
		if ($query -> num_rows > 0) {
	...
	{   
		...
		if(empty($row)){
			...
			$sql= "
			  ...
			  update
				  " . $this->db->dbprefix('product') . " p,
				  " . $this->db->dbprefix('server') . " s
			  set 
				  p.active = 0,
				  s.active = 0
			  where
				  p.id=s.product_id and p.user_id = s.user_id and p.id = $productId and p.user_id = $userId";
			$this->db->query($sql);
			$affect = $this->db->affected_rows();
			if ($affect > 0) {
				return true;
	...
	//获取当前用户所分配的游戏
	function getUserProductGame($userId)
	{
		$this->db->where('user_id',$userId);
		$query = $this->db->get('razor_user2product')->result();
		$list = array();
		foreach ($query as $value) {
			array_push($list,$value->product_id);
		}
		return $list;
	}
}