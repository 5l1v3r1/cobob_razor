...
<section id="main" class="column" style='height: 1500px;'>
	...
	<article class="module width_full">
		<header>
			<h3 class="tabs_involved"><?php echo  lang('m_kaiFuHuoDong_yao')?></h3>
		</header>
...
$('a.view').live('click',function(){
	...
	$(this).parents('tr').after('<tr id="add-iframe"><td colspan="11" style="height:600px;"><iframe src="<?php echo site_url() ?>/sysanaly/newserveractivity/charts?activity_issue='+activity_issue+'"  frameborder="0" scrolling="no" style="width:100%;height:100%;"></iframe></td></tr>');
})
$(document).ready(function(){
	$('.search-btn').bind('click',function(){
		var name = $('#filter-select option:selected').text();
		$.get("<?php echo site_url() ?>/sysanaly/newserveractivity/filter?name="+name,function(res){
