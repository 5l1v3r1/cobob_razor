 ...
 * @link     http://www.cobub.com
 */
class Channelimportamount extends CI_Controller
{
	private $data = array();
	...
	function index()
	{
		...
		$this->data['result'] = $this->channelimportamount->getChannelimportamountData($fromTime, $toTime, $channel, $server, $version);

		$this->common->requireProduct();
		$this->common->loadHeaderWithDateControl(lang('m_quDaoDaoLiang_yao'));
		
		$ruteName = $this->router->fetch_class();
		$canRead = $this->common->isDisplay($ruteName);
		if($canRead == '1'){
			$this->load->view('channelanaly/channelimportamountview',$this->data);
		}
		else{
	...
	function echarts(){
		...
		$this->load->view('widgets/channelimportamount',$this->data);
	}
}
