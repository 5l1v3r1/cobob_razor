 ...
 * @link     http://www.cobub.com
 */
class Paylevel extends CI_Controller
{
	private $data = array();
	...
	{	
		...
		$this->data['result'] = $this->paylevel->getpayleveldata($fromTime,$toTime,$channel,$server,$version);

		$this->common->requireProduct();
		$this->common->loadHeaderWithDateControl(lang('t_paylevel_analysis_yao'));
		
		$ruteName = $this->router->fetch_class();
		$canRead = $this->common->isDisplay($ruteName);
		if($canRead == '1'){
			$this->load->view('payanaly/paylevelview',$this->data);
		}
		else{
	...
	function charts(){
		...
		$this->load->view('widgets/paylevel',$this->data);
	}
}
