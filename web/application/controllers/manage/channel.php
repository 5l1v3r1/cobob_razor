...
class Channel extends CI_Controller
{
    ...
    function addsychannel()
    {
        ...
        $channel_name = $_POST['channel_name'];
        $platform = $_POST['platform'];
        $isUnique = $this->channel->isUniqueSystemchannel($channel_id,$channel_name, $platform);
        if (!empty($isUnique)) {
            echo false;
        } else {
            if ($channel_name != '' && $platform != '') {
                $this->channel->addsychannel($channel_id,$channel_name, $platform, $userid);
                echo true;
            }
        }
    }
    ...
    function editchannel($id)
    {
        $userid = $this->common->getUserId();
        $this->data['platform'] = $this->channel->getplatform();
        $this->data['edit'] = $this->channel->getdechaninfo($userid, $id);
        $this->data['guest_roleid'] = $this->common->getUserRoleById($userid);
    ...
    function modifychannel()
    {
        ...
        $channel_name = $_POST['channel_name'];
        $platform = $_POST['platform'];
        $type = $this->channel->getChannelType($edit_id);
        $isUnique = '';
        if ($type == "user") {
            $userid = $this->common->getUserId();
            $isUnique = $this->channel->isUniqueChannel($userid, $channel_name, $platform);
        ...
        if ($type == "system") {
            $isUnique = $this->channel->isUniqueSystemchannel($edit_id,$channel_name, $platform);
        }
        if ($channel_name != '' && $platform != '' && $channel_id != '') {
            $this->channel->updatechannel($edit_id, $channel_id, $channel_name, $platform, $channel_id);
            echo true;
        }
    }
