 ...
 * @link     http://www.cobub.com
 */
class Borderintervaltime extends CI_Controller
{
	private $data = array();
	...
	{	
		...
		$this->data['result'] = $this->borderintervaltime->getBorderintervaltime($fromTime,$toTime,$channel,$server,$version);
		$this->common->requireProduct();
		$this->common->loadHeaderWithDateControl(lang('t_startgap_analysis_yao'));
		
		$ruteName = $this->router->fetch_class();
		$canRead = $this->common->isDisplay($ruteName);
		if($canRead == '1'){
			$this->load->view('onlineanaly/borderintervaltimeview',$this->data);
		}
		else{
	...
	function charts(){
		...
		$this->load->view('widgets/borderintervaltime',$this->data);
	}
}
